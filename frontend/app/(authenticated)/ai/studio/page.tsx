...
import api, { type GeneratePostResponse, type GenerateImageResponse, type Product } from '@/lib/api';
...
export default function AIStudioPage() {
  ...
  const [activeTab, setActiveTab] = useState<TabId>(
    ['content', 'image'].includes(tabParam || '') ? (tabParam as TabId) : 'content'
  );
  ...
  useEffect(() => {
    if (tabParam && ['content', 'image'].includes(tabParam)) {
      ...
      <div className="px-4 sm:px-8 pt-9 pb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-white/50 text-sm mt-2">Generate content and images with AI</div>
          </div>
